...
export default async function CustomerExperienceLayout({
  children,
}: {
  ...
  const role = ((session.user as { role?: Role }).role ?? "cx_team") as Role;

  if (!canAccessDepartment(role, "customer_experience")) {
    redirect("/marketing");
  }
