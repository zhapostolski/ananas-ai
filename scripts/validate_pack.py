...
if agents.exists() and schedules.exists():
    ...
    if agent_names != sched_names:
        errors.append(f'Agent/schedule mismatch: agents={sorted(agent_names)} schedules={sorted(sched_names)}')
...
print('VALIDATION OK')
print(f'Checked {len(required_files)} required files, {len(json_files)} JSON files, and {len(subagents)} subagents.')
